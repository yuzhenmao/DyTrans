...
class PositionalEncoding(nn.Module):
    ...
    def forward(self, x):
        pe = torch.sin(self.embed(torch.arange(0, x.size(1), dtype=torch.long).cuda())).unsqueeze(0)
        x = x + pe
        return x

class TransformerPredictor(pl.LightningModule):

    def __init__(self, input_dim, model_dim, num_heads, num_layers, seq_len=0, dropout=0.0, norm_first=False):
        """
        Inputs:
    ...
    def _create_model(self):
        ...
        #     nn.Linear(self.hparams.model_dim*self.hparams.seq_len, self.hparams.model_dim),
