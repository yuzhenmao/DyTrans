...
class Logger:
    ...
    def __init__(
        ...
        while True:
            if with_timestamp:
                ...
                if index is None:
                    index = date
                self._experiment_name = experiment_name + "_" + index + date
            else:
                self._experiment_name = experiment_name + "_" + index
    ...
    def add_metric(self, name: str, value, epoch: int) -> None:
        ...
        if self._save_metrics:
            ...
            with open(path, "a") as f:
                if isinstance(value, float):
                    print("%d,%f" % (epoch, value), file=f)
                    if self._writer:
                        self._writer.add_scalar(f"metrics/{name}", value, epoch)
                else:
                    print(f"{epoch},", ','.join(['%f'%item for item in value]), file=f)
